...
patch_reviewer.py
-----------------
Run this ONCE on your machine to replace the Q_H LLM rubric
with the deterministic EARS compliance checker.

Usage:
    python patch_reviewer.py

This patches src/evaluation/tr_reviewer.py in-place.
No files to download or copy.
"""
import os, sys
...
Five-dimension quality review:
  ...
  3. Traceability
  4. Quality defects
  5. Schema suitability (Q_A)

Q_H uses deterministic EARS compliance (no LLM call).
A TR passes if it meets >= 4 of 5 criteria:
  (a) contains "shall"
  (b) has trigger word (When/If/While/Upon) or "the X shall" pattern
  (c) expected_outputs >= 4 words total
  (d) preconditions list non-empty
  (e) stimuli list non-empty
"""
...
class TRReviewer:
    ...
    def review(self, tr_doc, demand_model, verbose=True):
        ...
        if verbose: print("    -> Q_H={:.2f}  ({}/{} EARS-compliant)".format(
    ...
    def _dim4_quality(self, trs):
        ...
        untestable = [tr.tr_id for tr in trs
    ...
    def _ears_compliance(self, trs):
        trigger_p = re.compile(r"\\b(when|if|while|upon|on)\\b", re.IGNORECASE)
        subject_p = re.compile(r"\\bthe\\s+\\w+\\s+shall\\b", re.IGNORECASE)
